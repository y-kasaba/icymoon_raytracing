...
#include <stdio.h>
#include <stdlib.h>				/* standard C include statements */
#include <math.h>
#include <string.h>
#include <ctype.h>
  ...
  if ((argv == 2) && (*(argc[1]) == 'h'))
	{
	  ...
	  printf
		("   Coord: D - Geodetic (Altitude relative to Earth's surface.)\n");
	  printf
		("          C - Geocentric (Altitude relative to Earth's center.)\n");
	  ...
	  printf ("\nNote: \n");
	  printf ("   All arguments are optional but must prserve order.\n");
	  ...
	  while (stream == NULL)
		{
		  printf ("\n\n");
		  printf
			("What is the name of the model data file to be opened? ==> ");
...
/* New statement Dec 1999 changed by wmd  required by year 2000 models */
		  if (!strncmp (inbuff, "   ", 3))
			{					/* If 1st 3 chars are spaces */
			  ...
			  if (modelI > MAXMOD)
				{				/* If too many headers */
				  printf ("Too many models in file %s on line %d.", mdfile,
						  fileline);
				  fclose (stream);
				  exit (6);
				}
			  ...
			  sscanf (inbuff, "%s%f%d%d%d%f%f%f%f", model[modelI],
					  &epoch[modelI], &max1[modelI], &max2[modelI],
					  &max3[modelI], &yrmin[modelI], &yrmax[modelI],
					  &altmin[modelI], &altmax[modelI]);
	  ...
	  while (again == 2)
		{
		  ...
		  if (range == 1)
			  ...
			  while ((sdate < minyr) || (sdate > maxyr + 1))
				{
				  ...
				  if (decyears == 1)
					{
					  printf ("\nEnter the decimal date (%4.2f to %4.0f): ",
							  minyr, maxyr);
					  safegets (inbuff, MAXREAD);
					  sdate = atof (inbuff);
					}
				  else
					{
					  ...
					  while ((isyear > (int) maxyr + 1)
							 || (isyear < (int) minyr))
						{
						  printf ("\nEnter the date (%4.2f to %4.2f)\n ",
								  minyr, maxyr);
						  printf ("\n   Year (%d to %d): ", (int) minyr,
								  (int) maxyr);
						  safegets (inbuff, MAXREAD);
						  isyear = atoi (inbuff);
						}
		  ...
		  else
			{
			  ...
			  while ((sdate < minyr) || (sdate > maxyr))
				{
				  ...
				  if (decyears == 1)
					{
					  printf
						("\nEnter the decimal start date (%4.2f to %4.0f): ",
						 minyr, maxyr);
					  safegets (inbuff, MAXREAD);
					  sdate = atof (inbuff);
					}
			  ...
			  while ((edate <= sdate) || (edate > maxyr + 1))
				{
				  ...
				  if (decyears == 1)
					{
					  printf
						("\nEnter the decimal end date (%4.2f to %4.0f): ",
						 sdate, maxyr);
					  safegets (inbuff, MAXREAD);
					  edate = atof (inbuff);
					}
			  ...
			  while ((step <= 0) || (step > (edate - sdate)))
				{
				  printf ("\nEnter the step size in years. (0 to %4.2f): ",
						  edate - sdate);
				  safegets (inbuff, MAXREAD);
				  step = atof (inbuff);
				}
		  ...
		  while ((igdgc != 1) && (igdgc != 2))
			{
			  ...
			  printf
				("\n       1) Geodetic (From the surface of the Earth.)");
			  printf
		  ...
		  if (igdgc == 1)
			...
			units = 1;			/* geocentric always in km */
		  ...
		  while ((alt < minalt) || (alt > maxalt))
			{
			  if (igdgc == 2)
				printf
				  ("\n\nEnter geocentric altitude in km (%.2f to %.2f): ",
				   minalt, maxalt);
			  if (igdgc == 1 && units == 1)
				printf ("\n\nEnter geodetic altitude in km (%.2f to %.2f): ",
						minalt, maxalt);
			  if (igdgc == 1 && units == 2)
				printf
				  ("\n\nEnter geodetic altitude in meters (%.2f to %.2f): ",
				   minalt, maxalt);
			  if (igdgc == 1 && units == 3)
				printf
				  ("\n\nEnter geodetic altitude in feet (%.2f to %.2f): ",
				   minalt, maxalt);
			  safegets (inbuff, MAXREAD);
			  alt = atof (inbuff);
			}
			  ...
			  while ((latitude < -90) || (latitude > 90))
				{
				  ...
				  latitude =
					degrees_to_decimal (ilat_deg, ilat_min, ilat_sec);
			  ...
			  while ((longitude < -180) || (longitude > 180))
				{
				  ...
				  longitude =
					degrees_to_decimal (ilon_deg, ilon_min, ilon_sec);
	 ...
	 /** This will compute everything needed for 1 point in time. **/
		  ...
		  if (max2[modelI] == 0)
		  ...
		  else
			{
			  ...
			  nmax =
				extrapsh (sdate, epoch[modelI], max1[modelI], max2[modelI],
						  3);
			  nmax =
				extrapsh (sdate + 1, epoch[modelI], max1[modelI],
						  max2[modelI], 4);
			}
		  ...
		  if (range == 1)
		  ...
		  else
			{
			  ...
			  printf
				("\n   (yr)       (deg)        (deg)       (nt)    (nt)");
			  printf ("    (nt)    (nt)    (nt)\n");
			  printf ("  %4.2f %4dd %5.0fm %4dd %5.0fm ",
					  sdate, ddeg, dmin, ideg, imin);
			  printf ("%7.0f %7.0f %7.0f %7.0f %7.0f\n", h, x, y, z, f);


		   /** Optimization note **/
			  /* Because the model file is linear between boundaries, this
			   * loop could be replaced with a linear interpolation between
			   * model boundaries. Then inter/extrapsh would only have to be
			   * called when crossing a boundary.
			   * Similarly, getshc need only be called once for each model.
			   * However, quick attepts to do so didn't work.  Good luck =]
			   *                                         dio  6-6-96
			   */
				  ...
				  for (counter = 0; counter < step; counter++)
					{
					  if (max2[modelI] == 0)
						{		/*If not last element in array */
						  if (syr > yrmin[modelI + 1])
							{	/* And past model boundary */
							  modelI++;	/* Get next model */
							}
						}
					}
				  if (max2[modelI] == 0)
					{			/*If still not last element */
					  getshc (mdfile, 1, irec_pos[modelI], max1[modelI], 1);
					  getshc (mdfile, 1, irec_pos[modelI + 1],
							  max1[modelI + 1], 2);
					  nmax =
						interpsh (syr, yrmin[modelI], max1[modelI],
								  yrmin[modelI + 1], max1[modelI + 1], 3);
					  nmax =
						interpsh (syr + 1, yrmin[modelI], max1[modelI],
								  yrmin[modelI + 1], max1[modelI + 1], 4);
					}
				  else
					{
			  ...
			  printf ("              dD        dI       dH       dX       ");
			  printf ("dY       dZ        dF");
			  printf ("\n           (min/yr)  (min/yr)  (nT/yr)  (nT/yr)  ");
			  printf ("(nT/yr)  (nT/yr)   (nT/yr)");
			  printf ("\n  %4.2f  %7.0f  %7.0f  %7.0f", edate, ddot, idot,
					  hdot);
			  printf ("  %7.0f  %7.0f   %7.0f  %7.0f", xdot, ydot, zdot,
					  fdot);
			}
	  ...
	  if (range == 1)
	  ...
	  else
		{
		  if (decyears == 1)
			...
			{
			  printf ("%d,%d,%d-%d,%d,%d-%4.2f ", isyear, ismonth, isday,
					  ieyear, iemonth, ieday, step);
			}
		}
	  if (igdgc == 1)
		{
...
/*           elev      - altitude above mean sea level (igdgc=1), or       */
...
int
shval3 (igdgc, flat, flon, elev, nmax, gh, iext, ext1, ext2, ext3)
	 int igdgc;
	 float flat;
	 float flon;
	 float elev;
	 ...
	 float ext3;
{
  ...
  float dtr = 0.01745329; // == π/180 DegToRad?
  float slat;
  float clat;
  float ratio;
  float aa, bb, cc, dd;
		  ...
		  if (m == n)
			{
			  ...
			  cl[m] = cl[m - 1] * cl[1] - sl[m - 1] * sl[1]; // 加法定理。
			  // cl[1] == cos( flong ) なので、 cl[n] == cos( n*flong )になる。
			}
		  else
			{
